...
const useKOCProfileChartViews = ({ granularity }) => {
  const { id } = useParams();
  ...
  const { data, isLoading, isSuccess, isError } = useQuery({
    queryKey: [
      'koc-profile-chart-views',
      { kocIds: [id], granularity, from, to },
    ],
    queryFn: () =>
      getStatisticsContentsOfViews(granularity, { ownerId: id, from, to }),
    staleTime: 1000 * 10,
    select: (data) =>
...
export default useKOCProfileChartViews;
