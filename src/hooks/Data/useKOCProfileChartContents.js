...
import { getContentsStatistic } from '../../services/api';

const useKOCProfileChartContents = ({ granularity }) => {
  const { id } = useParams();
  ...
  const { data, isLoading, isSuccess, isError } = useQuery({
    queryKey: [
      'koc-profile-chart-contents',
      { kocIds: [id], granularity, from, to },
    ],
    queryFn: () =>
      getContentsStatistic({ kocIds: [id], granularity, from, to }),
    staleTime: 1000 * 10,
    select: (data) =>
...
export default useKOCProfileChartContents;
