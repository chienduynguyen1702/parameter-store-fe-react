...
import { getProductsStatistic } from '../../services/api';

const useKOCProfileChartProductsSold = ({ granularity }) => {
  const { id } = useParams();
  ...
  const { data, isLoading, isSuccess, isError } = useQuery({
    queryKey: [
      'koc-profile-chart-products-sold',
      { kocIds: [id], granularity, from, to },
    ],
    queryFn: () =>
      getProductsStatistic({ kocIds: [id], granularity, from, to }),
    staleTime: 1000 * 10,
    select: (data) =>
...
export default useKOCProfileChartProductsSold;
